import svgPaths from "../../imports/svg-xct8axrdwb";
...
function Frame1({ onSend }: { onSend: () => void }) {
  return (
    <button 
      onClick={onSend}
      className="bg-[rgba(10,24,21,0.32)] content-stretch flex flex-col items-center justify-center overflow-clip p-[8px] relative rounded-[50px] shrink-0 size-[32px] hover:bg-[rgba(10,24,21,0.42)] transition-colors cursor-pointer"
    >
      <Frame3 />
    </button>
  );
...
function Frame2({ onSend }: { onSend: () => void }) {
  return (
...
function Frame({ onSend }: { onSend: () => void }) {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <Frame4 />
      <Frame2 onSend={onSend} />
    </div>
  );
}
...
export default function ChatInput({ value, onChange, onSend }: ChatInputProps) {
  ...
  return (
    <div className="bg-white content-stretch flex flex-col gap-[4px] items-start justify-center px-[16px] py-[12px] relative rounded-[24px] w-full" data-name="Assistant Chat Box v2">
      ...
      <div className="content-stretch flex h-[50px] items-start justify-between overflow-clip relative shrink-0 w-full" data-name="Search">
        <input
          ...
          className="font-['Inter:Medium',sans-serif] font-medium leading-[1.6] not-italic shrink-0 text-[#919191] placeholder:text-[#919191] text-[14px] bg-transparent border-none outline-none w-full"
        />
      </div>
      <Frame onSend={onSend} />
    </div>
